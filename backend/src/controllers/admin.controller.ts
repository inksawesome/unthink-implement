...
export const createLeave = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (result.overlappingAppointments.length > 0) {
      const doctorTextBody = `Your leave for ${dateStr.toLocaleDateString()} has been processed. ${result.overlappingAppointments.length} appointments were automatically cancelled and patients have been notified.`;
      const doctorHtmlBody = buildEmailTemplate(
        'Leave Approved & Appointments Cancelled',
        `Your leave for ${dateStr.toLocaleDateString()} has been processed. <b>${result.overlappingAppointments.length}</b> appointments were automatically cancelled, and the patients have been notified.`,
        { 'Date': dateStr.toLocaleDateString(), 'Cancelled Appointments': result.overlappingAppointments.length.toString() }
      );
